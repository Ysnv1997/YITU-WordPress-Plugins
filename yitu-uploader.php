...
include("YITU-UPLOADER-WIDGETS.php");
...
function donate()
{
	echo '<!-- 益图图床WordPress工具，www.htm.fun -->';
	wp_enqueue_script('smms-donate', YITU_URL . 'js/donate.js', array(), YITU_VERSION, true);
}
//插件更新检测
function update()
{
...
function YITU_UPLOADER_LINKS($actions, $plugin_file)
{
	...
	if ($plugin == $plugin_file) {
		...
		$site_link	= array('support' => '<a href="https://github.com/qcgzxw/YITU-UPLOADER" target="_blank">使用说明</a>');
...
{
	add_options_page('YITU-UPLOADER设置页面', 'SM图床设置', 'manage_options', 'YITU-UPLOADER-OPTIONS', 'my_plugin_options');
}
...
function my_plugin_options()
{
	if (isset($_POST['Update'])) {
		...
		if ($ver > YITU_VERSION) {
			...
			echo '<div class="notice notice-warning"><p>YITU-UPLOADER 插件已经有新版本啦！ <a target="_blank" href="' . $url . '">立即下载</a></p><p><strong>更新内容：</strong>' . $content . '</p></div>';
		} else {
	...
	if (isset($_POST['DataSubmit'])) {
		...
		if (!isset($_POST['Update'])) {
			echo '<div class="updated subscribe-main" id="message"><p>创作不易，给GitHub点个Star吧。——<span class="text-ruo">[<a href="https://github.com/qcgzxw/YITU-UPLOADER" target="_blank">立即前往</a>]</span><i class="fr fb f20 qcgzxw-close">&#215;</i></p>';
	...
	echo '<div class="wrap">';
	echo '<h2>YITU-UPLOADER 插件设置</h2>';
	echo '<p>&nbsp;&nbsp;YITU-UPLOADER是一款为WordPress添加上传图片小工具以及评论处图片上传按钮的插件！</p>';
